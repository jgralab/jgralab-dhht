...
public abstract class VertexProxy extends
		GraphElementProxy<VertexClass, Vertex, EdgeClass, Edge> implements Vertex {
	...
	public Incidence getFirstIncidence(Graph traversalContext) {
		Incidence firstIncidence = localGraphDatabase
				.getIncidenceObject(localGraphDatabase
						.getFirstIncidenceIdAtVertexId(elementId));
	...
	public Incidence getLastIncidence(Graph traversalContext) {
		Incidence lastIncidence = localGraphDatabase
				.getIncidenceObject(localGraphDatabase
						.getLastIncidenceIdAtVertexId(elementId));
	...
	public final Incidence getFirstIncidenceToEdge(Class<? extends Edge> edgeClass, Direction direction) {
		return getFirstIncidenceToEdge(localGraphDatabase.getTraversalContext(), edgeClass, direction);
	}
	...
	public Incidence connect(IncidenceClass incidenceClass, Edge elemToConnect,
			long incidenceId) {
		if (elemToConnect instanceof EdgeImpl)
		return  ((EdgeImpl)elemToConnect).connect(incidenceClass, this,
				incidenceId);
		else
			return  ((EdgeProxy)elemToConnect).connect(incidenceClass, this,
					incidenceId);
	}
