...
public interface GraphElement extends AttributedElement {
	...
	/**
	 * Returns the first incident {@link GraphElement} of this
	 * {@link GraphElement}. If such an {@link GraphElement} does not exist,
	...
	/**
	 * Returns the first incident {@link GraphElement} of this
	 * {@link GraphElement} with direction <code>direction</code>. If such an
	 * {@link GraphElement} does not exist, <code>null</code> is returned.
	 * 
	 * @param direction
	 *            {@link Direction} of connected {@link Incidence}s. If set to
	 *            <code>null</code> {@link Incidence}s of both directions are
	...
	/**
	 * Get the first incident {@link GraphElement} which is connected via an
	 * {@link Incidence} which has one of the aggregation semantics given by
	 * <code>incidentTypes</code> at either this {@link Incidence} (
	 * <code>thisIncidence == true</code>) or that {@link Incidence} (
	 * <code>thisIncidence == false</code>). If there are no
	 * <code>incidentTypes</code> given, it simply returns the first incident
	 * {@link GraphElement}. If such an {@link GraphElement} does not exist,
	 * <code>null</code> is returned.<br/>
	 * <br/>
	 * For example, this returns the first incident {@link GraphElement} to a
	 * parent {@link GraphElement} in the containment hierarchy.
	 * 
	 * <pre>
	 * ge.getFirstIncidence(true, IncidenceType.AGGREGATION, IncidenceType.COMPOSITION)
	 * </pre>
	 * 
	 * And this returns the first incident {@link GraphElement} to a child
	 * {@link GraphElement} in the containment hierarchy.
	 * 
	 * <pre>
	 * v.getFirstIncidence(false, IncidenceType.AGGREGATION, IncidenceType.COMPOSITION)
	 ...
	 * @return {@link GraphElement}
	 */
	public GraphElement getFirstIncidentGraphElement(boolean thisIncidence,
			IncidenceType... incidentTypes);

	/**
	 * Returns the first incident GraphElement in lambda-seq where the
	 * corresponding {@link GraphElement} is of class
	 * <code>aGraphElementClass</code>. If such an {@link GraphElement} does not
	 * exist, <code>null</code> is returned.
	 * 
	 * @param aGraphElementClass
	 *            {@link GraphElementClass} to search for
	 * @return {@link GraphElement}
	 */
	public GraphElement getFirstIncidentGraphElement(
	 ...
	 * Returns the first incident GraphElement in lambda-seq where the
	 * corresponding {@link GraphElement} is of class
	 * <code>aGraphElementClass</code>. If <code>noSubclasses</code> is set to
	 * <code>true</code> the incident {@link GraphElement} must not be an
	 * instance of a subclass of <code>aGraphElementClass</code>. If such an
	 * {@link GraphElement} does not exist, <code>null</code> is returned.
	 * 
	 * @param aGraphElementClass
	 *            {@link GraphElementClass} to search for
